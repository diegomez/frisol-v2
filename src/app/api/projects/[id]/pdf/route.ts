...
export async function GET(request: Request, { params }: { params: { id: string } }) {
  ...
  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><style>
    *{margin:0;padding:0;box-sizing:border-box}body{font-family:Arial,sans-serif;font-size:11px;color:#1f2937;line-height:1.5}
    ...
    <div class="client"><span class="lbl">ID Interno:</span> PRJ-${String(project.projectNumber || 0).padStart(5, '0')}${maxUrgencia ? ` <span class="badge badge-u-${maxUrgencia}">&#9889; URGENTE (${maxUrgencia.toUpperCase()})</span>` : ''}<br><span class="lbl">Proyecto:</span> ${esc(project.nombreProyecto || 'Sin nombre')}<br><span class="lbl">Cliente:</span> ${esc(project.nombreCliente || 'Sin cliente')}<br><span class="lbl">CRM:</span> ${esc(project.crmId || '—')}<br><span class="lbl">Tribu:</span> ${esc(project.tribe?.name || '—')}<br><span class="lbl">Fecha:</span> ${fmtDate(project.fechaInicio)}${project.importancia != null ? `<br><span class="lbl">Importancia:</span> ${project.importancia}/5` : ''}${project.pedido ? `<br><span class="lbl">Pedido:</span> ${esc(project.pedido)}` : ''}</div>
